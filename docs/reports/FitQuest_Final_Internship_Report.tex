...
detector metrics in Section~\ref{sec:results} cannot support a claim of
generalization. Reporting across several seeds would additionally give a variance
estimate.
...
that the contribution of each to usable detection quality is measured rather than
assumed.
...
with repetition counts against manual ground truth, would establish counting
accuracy and calibration transferability together.
...
camera angle, lighting, and occlusion conditions --- together with a negative set
containing watches, phones, bottles, and bare hands, would provide both better
coverage and a defined denominator for measuring false positives.
